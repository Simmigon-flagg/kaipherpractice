...
let lastSpeak = null; // fn that replays the most recent audio (prompt or context phrase) for the 🔁 Repeat button
...
function renderSoundSelect() {
  ...
  if (!all.includes(winSoundame)) winSoundName = "Chime F5 (single)";
...
function startSpeedRound() {
  ...
  document.getElementById("subTitle").textContent = "Hear it, pick the meaning — ding = point";
...
function renderHome() {
  ...
  if (!inJudge()) {
    ...
    sd.innerHTML = '<span class="emoji">⚡</span><div class="name">Speed Round</div>' +
      '<div class="count">' + known + ' known ' + deckUnit() + ' · ' + SPEED_SECONDS + 's · hear it, pick the meaning · best: ' +
...
function nextQuestion() {
  ...
  if (speedMode) {
    // speed round: HEAR the Tagalog word, pick its English meaning. Tagalog voice only — never English.
    wd.textContent = tl; wd.className = "tl-word";
    lastSpeak = () => speak(tl);
    if (playBtn) playBtn.style.display = "block"; // 🔁 Repeat replays the Tagalog
    speak(tl);
    return;
  }
...
function finishAnswer(ok) {
  ...
  if (ok) {
    ...
    if (judge) {
      ...
      lastSpeak = () => speak(tl);                       // Repeat now replays the Tagalog you just learned
      if (!rev && phrasesOn() && PHRASE_DATA[tl]) {
        ...
        lastSpeak = () => speak(ptl);                    // …or the context phrase, if one is showing
        extraDelay = 1600;
      }
    }
  } else {
...
document.getElementById("playBtn").onclick = () => {
  if (typeof lastSpeak === "function") return lastSpeak(); // replay the most recent audio (word / sentence / phrase)
  if (!current) return;
